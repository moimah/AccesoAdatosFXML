...
public class AccesoAdatosController2 implements Initializable{
	...
	public AccesoAdatosController2() throws IOException {
  ...
  private boolean moverFile(String fromFile, String toFile) {
	...
	/**
	 * Metodo copiar carpetas y ficheros de manera recursiva	 
	 * @param d1
	 * @param d2
	 * @return exito
	 */   
  private boolean copiadoRecursivo(File d1, File d2) {
		...
		//Cambiar el File destino añadiendo el nombre	
		d2  = new File(destino);
		
		System.out.println("DESTINO " + destino);
		
		//Comprobar si existe, si es asi añadirle al path - copy + i	
		File f_aux = d2;
		File d3 = null;
		
		boolean existe = d2.exists();
		
		if(existe) { //Si existe el fichero entrar en un bucle que creará un nuevo fichero paralelo que no exista
			
			for(int i=0; i<100000; i++){ //No permitir más de 100000 Copias
		...
		if(!d3.exists()) { //Asignar valor de d3 a d2
			d2 = d3;
		}
