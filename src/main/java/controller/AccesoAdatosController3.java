...
public class AccesoAdatosController3 implements Initializable{
	...
	public AccesoAdatosController3() throws IOException {
	...
	public void clickBtnFicheros(ActionEvent event) {		
		...
		model.txtContenidoProperty().bind(lvFicheros.getSelectionModel().selectedItemProperty());
	...
	public void clickBtnContenido(ActionEvent event) {
		...
		String seleccionado = model.FicheroSeleccionadoProperty().get();		
		String path = ruta_actual + "\\" + seleccionado;
		
		File fichero = new File(path);
		
				
		if(fichero.exists()) { //Si la ruta es mayor a 2 (Minimo windows) 	
